...
let all_products_bught = document.querySelector(".all-products-bught")
let all_products_new = document.querySelector(".all-products-new")

let filterDiscount = products.filter((el) => el.discount > 0)
filterDiscount.map((el) => {
    all_products_discount.innerHTML += `
       <div
...
let filterNew = products.filter((el) => el.discount === 0)
filterNew.map((el) => {
    all_products_new.innerHTML += `
         <div
                        class="max-w-[302px] rounded-[4px] w-full cursor-pointer bg-[white] relative group overflow-hidden shadow-2xl">
                        <a href="./single.html">
                            <img class="w-full h-[140px] sm:h-[202px]" src=${el.images[0]} alt="img">
